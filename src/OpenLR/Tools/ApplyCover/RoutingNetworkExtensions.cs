...
public static class RoutingNetworkExtensions
{
    ...
    /// </summary>
    /// <param name="mutator">The routing network mutator.</param>
    /// <param name="edges">The edges.</param>
    /// <param name="rewrite">The callback to rewrite attributes.</param>
    public static void ApplyCover(this RoutingNetworkMutator mutator,
        IEnumerable<(EdgeId edge, bool forward, ushort tailOffset, ushort headOffset)> edges,
        RewriteEdgeAttributesFunc rewrite)
    {
        ...
        foreach (var covered in edges)
        {
            ...
            {
                // add between segment.
                var attributes = rewrite(edgeEnumerator.Attributes, edgeEnumerator.Forward).ToList();
